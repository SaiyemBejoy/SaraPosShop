...
namespace DAL.Repository
{
    public class ReportRepository : ApplicationDbContext, IReportRepository
    {
        ...
        public async Task<DataSet> GetallShopTransitReturnItem(string challanNo)
        {
            List<OracleParameter> param = new List<OracleParameter>
            {
                new OracleParameter(":TRANSIT_RETURN_CHALLAN_NUM", OracleDbType.Varchar2, challanNo, ParameterDirection.Input)
            };

            var query = "SELECT " +
                        "'Transit Return Product Challan Details 'RPT_TITLE," +
                        ...
                        "SALE_PRICE," +
                        "QUANTITY " +
                        "FROM VEW_RPT_TRNSIT_RTN_PROD_DTAILS where TRANSIT_RETURN_CHALLAN_NUM = :TRANSIT_RETURN_CHALLAN_NUM ";

            var ds = await GetDataThroughDataSetRptAsync(query, param, "VEW_RPT_TRNSIT_RTN_PROD_DTAILS");
            return ds;
        }


        #region "Transit Stock Report"
        public async Task<DataSet> TransitStock(TransitReportModel objTransitReportModel)
        {
            ...
            var query = " SELECT " +
                        "'Transit Stock Report' RPT_TITLE," +
                        "BRAND_NAME," +
                        "CENTRAL_BIN," +
                        "FROM_SHOP_NAME," +
                        "FROM_SHOP_ADDRESS," +
            ...
            "from VEW_RPT_TRANSIT_PRODUCT_INFO where 1=1 ";

            if (objTransitReportModel.MarketPalceId > 0)
            {
                query += " AND  MARKET_PLACE_ID = '" + objTransitReportModel.MarketPalceId + "' ";

            }
            if (!string.IsNullOrWhiteSpace(objTransitReportModel.StyleName))
            {
                query += " AND  STYLE_NAME LIKE '" +"%"+objTransitReportModel.StyleName.ToUpper()+"%"+ "' ";

            }

            ds = await GetDataThroughDataSetRptAsync(query, param, "VEW_RPT_TRANSIT_PRODUCT_INFO");
            return ds;
        }
        ...
        public async Task<DataSet> GenerateVoidInvoiceHistory(SaleReportModel objSaleReportModel)
        {
            ...
            var query = " SELECT " +
                        " ' Void Invoice History  between ' || to_date( '" + objSaleReportModel.FromDate + "', 'dd/mm/yy') || ' to '|| to_date( '" + objSaleReportModel.ToDate + "', 'dd/mm/yy')  RPT_TITLE, " +
                        "SHOP_NAME," +
                        "SHOP_ADDRESS," +
                        "SALE_INFO_ID," +
                        "INVOICE_NUMBER," +
                        ...
                        "SALESMAN_NAME," +
                        "PAYMENT_INFO " +
                        "from VEW_RPT_VOID_HISTORY where TRUNC (INVOICE_DATE) BETWEEN  to_date(:INVOICE_FROM_DATE, 'dd/mm/yyyy') AND  to_date(:INVOICE_TO_DATE , 'dd/mm/yyyy') ";
            
            ds = await GetDataThroughDataSetRptAsync(query, param, "VEW_RPT_VOID_HISTORY");
            return ds;
        }
        #endregion
    }
